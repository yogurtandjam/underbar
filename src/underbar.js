(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if(iterator){
      _.filter(array, function(item){
        ...
        if(newArr.includes(item) === false && truthyArr.includes(iterator(item)) === false){
          console.log(newArr)
          truthyArr.push(iterator(item))
          newArr.push(item);
        }
      })
    } else {
  ...
  _.memoize = function(func) {
    ...
    var alreadyCalled = false;
    var grab;
    // var test = func.apply(this, arguments);
    return function(x){
      if(!alreadyCalled || JSON.stringify(x) !== grab){
        grab = JSON.stringify(x);
        result = func.apply(this, arguments);
        alreadyCalled = true;
      }
      return result;
  ...
  _.delay = function(func, wait, arg1, arg2) {
    return setTimeout(function(){func(arg1, arg2)}, wait)
  };
  // _.delay = function(func, wait) {
  //   tempArr = arguments;
  //   return setTimeout(function(){func(input)}, wait)
  // };
